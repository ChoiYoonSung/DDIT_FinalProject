																...
																<table id="data-table-combine"
																	...
																	<thead>
																		<tr role="row" style="text-align: center;">
																			...
																			<th width="48%" class="text-nowrap sorting">제목</th>
																			<th width="10%" class="text-nowrap sorting">작성자</th>
																			<th width="12%" class="text-nowrap sorting">등록일</th>
																			<th width="5%" class="text-nowrap sorting">조회수</th>
																			<th width="5%" class="text-nowrap sorting">추천수</th>
																	...
																	<tbody>
																		...
																		<%
																			...
																			List<TotalKnowledgeVO> tkList = (List<TotalKnowledgeVO>)dataMap.get("tkList");
																			String tkCode = "";
																			for (int i = 0; i < tkList.size(); i++) {
																				tkCode = tkList.get(i).getTkCode();
																		%>
																		<tr style="text-align: center;" 
																			onclick="OpenWindow('detail.do?tkCode=<%=tkList.get(i).getTkCode()%>','상세보기',880,850);">
																			...
																			<td class="fw-bold text-inverse dtr-control"
																				...
																				<div
																					style="position: absolute; height: 1px; width: 0px; overflow: hidden;">
																					<input type="text" tabindex="0">
																				</div><%=tkList.get(i).getTkCode()%></td>
																			<td style="text-align: left;"><%=tkList.get(i).getTkTitle()%></td>
																			<td><%=tkList.get(i).getEmpName()%></td>
																			<td><%=tkList.get(i).getTkRegdate()%></td>
																			<td><%=tkList.get(i).getTkViewCnt()%></td>
																			<td><%=tkList.get(i).getTkLikeCnt()%></td>
																			<td>
																				<%
																					if (tkList.get(i).getAttachList().size() > 0) {
