...
function isGaramApp() {
	...
	return (userAgent.indexOf('mobileapp') != -1 || userAgent.indexOf('iosapp') != -1);
}

/**
 * 아래의 형식의 주소를 반환
 * Scheme(Protocal) + // + Host + Port
 * @param location
 */
...
/**
 * 아래의 형식의 주소를 반환
 * Scheme(Protocal) + // + Host + Port + Path + Query Parameter
 * @param location
 */
function getURI(location) {
	return getHost(location)+location.pathname+location.search;
}
...
function OpenWindow(UrlStr, WinTitle, WinWidth, WinHeight) {
	...
	if (isGaramApp()) {
		if (UrlStr.indexOf("http") != 0) {
			UrlStr = getHost(location) + UrlStr;
		}
		parent.location.href = getURI(parent.location) + '#popview|' + UrlStr;
	} else {
...
//카테고리ID Formatting CXXX / bbs_manager_view.jsp, bbs_admin_form.jsp
    ...
    var beforePrint = function() {
        $("#ActionButton,#btntbl,#breadcrumbs").css("display" , "none");
...
//        	$("#apprdoc_footer").css("display", "none");
	...
	(function() {
	    ...
	    var afterPrint = function() {
	        $("#ActionButton,#btntbl,#breadcrumbs").css("display" , "");
			// 공문인쇄 추가조건
	        var gongmoonHeader = $(".gongmoon");
	        if (gongmoonHeader.length > 0) {
...
//새창인 경우, 아닌 경우 처리 2013.06.10 김정국
...
function i18n(msg) {
	...
	if (top != this) {
		return eval("top.i18n(" + args + ")");
	} else if (opener != null) {
		return eval("opener.i18n(" + args + ")");
	}	
	if (typeof jQuery.i18n === "undefined")  {	//2020-11-11 추가
	    importNodeJs("/common/libs/jquery-i18n-properties/1.2.7/jquery.i18n.properties.min.js", function () {
	    	$.i18n.properties({
				name:'messages',
				path:'/common/',
				mode:'map',
