...
/*
	...
	*/
	...
	public static List<PAAttachVO> parseFileNameFromPAAttaches(List<PAAttachVO> attachList,String delimiter){
		// 프로젝트 자료실
		List<PAAttachVO> renamedAttachList = new ArrayList<PAAttachVO>();
		
		if(attachList != null) {
			for(PAAttachVO attach : attachList) {
				String fileName = attach.getPaAtName();//DB상의 fileName
				fileName = parseFileNameFromUUID(fileName, delimiter); //uuid가 제거된 fileName
